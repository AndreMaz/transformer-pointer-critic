...
[EdgeX Foundry](https://www.edgexfoundry.org/) is microservices-based project that aims to provide computation near at the edge. This project in particular defines a custom rule engine API that shares similarities with the `Rule` API.
...
[StarlingX](https://www.starlingx.io/) is a project specifically designed for IoT applications located at the edge. StarlingX is a virtualization manager that aims to simplify the management, orchestration and scaling of distributed edge-cloud computing devices.

Regardless of the number of nodes currently running (due to autoscaling), reverse proxies still perform the load balancing, of the incoming requests, across all the available nodes. These load balancing strategies adapt to the dynamics of the system.
